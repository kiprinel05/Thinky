...
import '../../domain/animals_lesson_content.dart';

class AnimalsLearningView extends StatefulWidget {
  final VoidCallback onDone;

  const AnimalsLearningView({super.key, required this.onDone});

  @override
  State<AnimalsLearningView> createState() => _AnimalsLearningViewState();
}

class _AnimalsLearningViewState extends State<AnimalsLearningView> {
  ...
  void _nextPage() {
    if (_currentPage < animalsLessonCards.length - 1) {
  ...
  Widget build(BuildContext context) {
    ...
    final isLast = _currentPage == animalsLessonCards.length - 1;
            ...
            children: [
              ...
              Expanded(
                child: PageView.builder(
                  ...
                  itemCount: animalsLessonCards.length,
                  onPageChanged: _onPageChanged,
                  itemBuilder: (context, index) {
                    final card = animalsLessonCards[index];
                    return _LessonPage(
                      card: card,
                      index: index,
                      colors: colors,
  ...
  Widget _buildProgressBar(AppColorsExtension colors) {
    final progress = (_currentPage + 1) / animalsLessonCards.length;
    return Padding(
      ...
      child: Column(
        children: [
          Row(
            ...
            children: [
              Text(
                '${_currentPage + 1} / ${animalsLessonCards.length}',
                style: GoogleFonts.alata(
                  fontSize: 12,
                  fontWeight: FontWeight.w700,
  ...
  Widget _buildDotIndicator() {
    return Row(
      ...
      children: List.generate(animalsLessonCards.length, (i) {
